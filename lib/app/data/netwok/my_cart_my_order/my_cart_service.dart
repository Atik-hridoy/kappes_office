...
class CartService extends GetConnect {
  ...
  Future<GetCartModel> fetchCartData(String token) async {
    try {
      ...
      if (token.isEmpty) {
        AppLogger.error('❌ [CartService] Error: Empty token provided');
      ...
      if (response.statusCode == 200) {
        if (response.data == null) {
          AppLogger.error('❌ [CartService] Error: Response data is null');
        ...
        try {
          ...
          AppLogger.error('❌ [CartService] Error parsing cart data: $e');
          AppLogger.error('❌ [CartService] Raw response: ${response.data}');
          throw Exception('Failed to parse cart data: $e');
        }
      } else {
        AppLogger.error('❌ [CartService] Server error: ${response.statusCode} - ${response.statusMessage}');
        AppLogger.error('❌ [CartService] Response data: ${response.data}');
        throw Exception('Server responded with status: ${response.statusCode}');
      }
    } on DioException catch (dioError) {
      AppLogger.error('❌ [CartService] Dio Error: ${dioError.type}');
      AppLogger.error('❌ [CartService] Error message: ${dioError.message}');
      AppLogger.error('❌ [CartService] Error response: ${dioError.response?.data}');
      AppLogger.error('❌ [CartService] Error request: ${dioError.requestOptions.uri}');
      throw Exception('Network error: ${dioError.message}');
    } catch (e) {
      AppLogger.error('❌ [CartService] Unexpected error: $e');
      AppLogger.error('❌ [CartService] Stack trace: ${StackTrace.current}');
      rethrow;
    }
  }
}
